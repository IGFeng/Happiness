...
<div id="main">
    <div id="submit">
    <form name="form1" method="post" action="{{url('rereplysolve',['mid'=>$content->mid,'mastername'=>$content->nickname])}}">
        @csrf
    <div style="font-weight: 600;color:#4f98ca;" id="reply_nol">
    {{$content->nickname}}<span style="color:#999;font-weight:normal">于{{$content->replytime}}回复</span>{{$content->mastername}}:
    </div>
